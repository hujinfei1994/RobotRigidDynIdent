...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    double *ptr_T = mxGetPr(plhs[0]);
    double *ptr_err_code = mxGetPr(plhs[1]);

    double *ptr_jm = mxGetPr(prhs[0]);
    double *ptr_am = mxGetPr(prhs[1]);
    double *ptr_vm = mxGetPr(prhs[2]);
    double *ptr_p0 = mxGetPr(prhs[3]);
    double *ptr_p1 = mxGetPr(prhs[4]);
    double *ptr_Tt = mxGetPr(prhs[5]);
     
    HKA_STATUS sts = HKA_STS_OK; //输出接口变量
    TRAJ_INPUT input; //输入接口变量
