%% jnt = 4
% 待测试轴为4轴(jnt)，匀速往复；随动加减速轴为1轴(ax)，不同加速度往复；非零位静止轴为3轴(bx)，静止在-90度上。
...
pos_neg_jnt = deg2rad(-50);
pos_pos_jnt = deg2rad(50);
am_jnt = deg2rad(100);
vm_jnt = deg2rad(20);
...
pos_neg_ax = deg2rad(-100);
pos_pos_ax = deg2rad(100);
% 起始终止段
am_ax0 = deg2rad(80);
...
%% 联动轴加速往复段轨迹设计（4轴对应的联动加速轴是1轴）
index_n2p_number=zeros(acc_num,1);
index_p2n_number=zeros(acc_num,1);
for i = 1:acc_num
    % n2p
...
p1 = pos_bx0;
am = deg2rad(80);
vm = am*vel_acc_rate_bx0;
jm = am*jerk_acc_rate_bx0;
Tt = 3;
[T,err] = scurve_get_time(jm,am,vm,p0,p1,Tt);
if(err~=0)
    error('err');
...
m = length(save_pos);
save_q(1:m,jnt_bx) = save_pos;
save_q(m+1:end,jnt_bx) = save_pos(end);
save_q_dot(1:m,jnt_bx) = save_vel;
save_q_ddot(1:m,jnt_bx) = save_acc;
...
m = length(save_pos);
save_q(end-m+1:end,jnt_bx) = save_pos;
save_q_dot(end-m+1:end,jnt_bx) = save_vel;
save_q_ddot(end-m+1:end,jnt_bx) = save_acc;
...
save(['time_inertial_ident_J',num2str(jnt),'_A2.mat'],'save_T_neg2pos','save_T_pos2neg','save_T0','acc_data','acc_num','index_n2p','index_p2n','index_n2p_number','index_p2n_number','save_T_neg2pos_ax','save_T_pos2neg_ax','T_total')
